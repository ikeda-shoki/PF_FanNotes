class SearchController < ApplicationController
  def search
    ...
    if @search_word.empty?
      ...
      if @user_or_image === "1"
        @search_users = Kaminari.paginate_array(User.search_keyword(params[:search])).page(params[:page]).per(5)
      elsif @user_or_image === "2"
        @search_post_images = Kaminari.paginate_array(PostImage.preload(:user).search_keyword(params[:search])).page(params[:page]).per(8)
      end
    end
  end
